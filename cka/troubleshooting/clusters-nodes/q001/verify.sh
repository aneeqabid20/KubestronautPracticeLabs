...
if node_exec "$NODE_ALIAS" true >/dev/null 2>&1; then
  pass "Administrative SSH access to $K8S_NODE works"
else
  fail "Administrative SSH access to $K8S_NODE works"
fi
...
if [[ "$ready_status" == "True" ]]; then
  ...
  fail "$K8S_NODE is Ready (current Ready condition: ${ready_status:-unknown})"
fi
